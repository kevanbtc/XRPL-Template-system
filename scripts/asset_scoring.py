...
def score_asset(asset, weights, gates, thresholds):
    ...
    if vf < gates["min_vf_for_near_term"] or cc < gates["min_cc_for_near_term"]:
        ...
        gating_notes.append(
            f"Gated below Near-Term (VF={vf}, CC={cc}, was {composite_before:.2f})"
        )
    ...
    if verification_days > gates["verification_penalty_days"]:
        ...
        gating_notes.append(
            f"Verification delay penalty -{gates['verification_penalty_points']} (days={verification_days}, was {composite_before:.2f})"
        )
...
def generate_focus_md(
    ...
    with path.open("w", encoding="utf-8") as md:
        ...
        for r in nearterm:
            md.write(
                f"- {r['name']} — {r['composite']:.2f} | Unlocks: appraisal/counterparties/docs\n"
            )
