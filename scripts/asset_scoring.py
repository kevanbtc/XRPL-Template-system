...
def score_asset(asset, weights, gates, thresholds):
    ...
    if vf < gates['min_vf_for_near_term'] or cc < gates['min_cc_for_near_term']:
        ...
        gating_notes.append(f"Gated below Near-Term (VF={vf}, CC={cc}, was {composite_before:.2f})")
    ...
    if verification_days > gates['verification_penalty_days']:
        ...
        gating_notes.append(f"Verification delay penalty -{gates['verification_penalty_points']} (days={verification_days}, was {composite_before:.2f})")
    ...
    if (ttc == 0) or (vf <= 1) or (cc <= 1) or (ld <= 1 and ltv <= 1):
        ...
        gating_notes.append(
            f"REJECT: Unviable for near-term liquidity (TTC={ttc}, VF={vf}, CC={cc}, LD={ld}, LTV={ltv}; was {composite_before:.2f})"
        )
    else:
...
def compute_scores(assets, weights_doc):
    ...
    scored = []
    for a in assets:
        composite, bucket, contributions, gating_notes = score_asset(a, weights, gates, thresholds)
        row = {
...
def write_outputs(scored, csv_path: Path, md_path: Path, print_stdout: bool = True):
    ...
    def fmt(row):
        return [row['name'], f"{row['composite']:.2f}", row['bucket']] + [str(row.get(c,0)) for c in LANGUAGE_FIELDS]
    ...
    bucket_actions = {
        'Immediate': 'Commit execution resources now (legal finalize, commercialization, compliance monitoring). Allocate ~40% bandwidth.',
        'Near-Term': 'Advance verification & packaging (data room, appraisals, counterparties). Target readiness within next sprint.',
        'Background': 'Maintain light progress & monitoring. Improve documentation opportunistically.',
        'Archive': 'Park with no active spend. Revisit only if new verifiable evidence appears.',
        'Reject': 'Hard pass. Do not spend time or money until evidence dramatically improves (verification, compliance, liquidity, or time-to-cash).'
    }

    with md_path.open('w', encoding='utf-8') as md:
        md.write('# Asset Liquidity Scores\n\n')
        md.write('Composite scores after gating & penalties. Higher = faster + more efficient path to near-term cash.\n\n')
...
def generate_focus_md(scored, path: Path, top_immediate: int = 3, top_nearterm: int = 3):
    ...
    with path.open('w', encoding='utf-8') as md:
        ...
        if not nearterm:
            md.write('- None\n')
        for r in nearterm:
            md.write(f"- {r['name']} — {r['composite']:.2f} | Unlocks: appraisal/counterparties/docs\n")
...
def parse_args():
    ...
    p.add_argument('--weights', type=str, default=str(WEIGHTS_FILE), help='Path to weights JSONC')
    p.add_argument('--assets', type=str, default=str(DATA_FILE), help='Path to assets JSON')
    p.add_argument('--csv', type=str, default=str(CSV_FILE), help='Path to CSV output')
    p.add_argument('--md', type=str, default=str(MD_FILE), help='Path to Markdown output')
    p.add_argument('--focus-md', type=str, default=str(OUTPUT_DIR / 'Focus.current.md'), help='Path to Focus summary Markdown output')
    p.add_argument('--no-stdout', action='store_true', help='Do not print table to stdout')
    ...
    p.add_argument('--print-focus', action='store_true', help='Print Immediate/Near-Term summary to stdout')
    return p.parse_args()
...
def main():
    ...
    write_outputs(scored, Path(args.csv), Path(args.md), print_stdout=(not args.no_stdout))
