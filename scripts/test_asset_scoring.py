...
weights_doc = mod.strip_jsonc(
    ROOT / "docs" / "templates" / "scoring" / "AssetScoringWeights.jsonc"
)
...
assert (
    names["Unykorn IP Licensing"]["bucket"] == "Immediate"
), "Unykorn should be Immediate"

# Rejects should include IQD and Historic Chinese Bonds
assert names["IQD Boxes"]["bucket"] == "Reject", "IQD Boxes should be Reject"
assert (
    names["Historic Chinese Bonds"]["bucket"] == "Reject"
), "Historic Chinese Bonds should be Reject"
